...
(function(exports) {
	...
	function TcpServer(addr, port, options) {
		...
		// server socket (one server connection, accepts and opens one socket per
		// client)
	...
	TcpServer.prototype._onAccept = function(info) {
		...
		var tcpConnection = new TcpConnection(info.clientSocketId, this);
	 ...
	 */
	function TcpConnection(socketId, tcpServer) {
	...
	TcpConnection.prototype.addDataReceivedListener = function(callback) {
		// If this is the first time a callback is set, start listening for incoming
		// data.
	...
	TcpConnection.prototype._onReceiveError = function(info) {
		if (this.socketId != info.socketId)
			return;
		console.log("close socket:"+this.socketId);
		this.close();
		var i = 0;
		for (i = 0; i < this.tcpServer.openSockets.length; i++) {
			if(this.tcpServer.openSockets[i] == this){
				break;
			}
		}
		if(i==this.tcpServer.openSockets.length){
			console.log("no match size:"+i);
		}else{
			console.log("remove socket:"+this.socketId);
			this.tcpServer.openSockets.splice(i,1);
		}
	};
